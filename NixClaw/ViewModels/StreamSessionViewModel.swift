...
class StreamSessionViewModel: ObservableObject {
  ...
  private func startIPhoneSession() {
    ...
    // Wire up background mode callbacks
    geminiSessionVM?.onPauseVideoCapture = { [weak self] in
      self?.iPhoneCameraManager?.pauseCapture()
    }
    geminiSessionVM?.onResumeVideoCapture = { [weak self] in
      self?.iPhoneCameraManager?.resumeCapture()
    }
